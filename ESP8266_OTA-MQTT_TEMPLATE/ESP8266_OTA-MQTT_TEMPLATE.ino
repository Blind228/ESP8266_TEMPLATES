/*
  ...
  There is also a "ESP8266_OTA-MQTT_DEBUG-TEMPLATE" version on the discord
  that includes serial output.
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  // ****************INSERT MQTT ACTIVATED COMMANDS HERE********************
...
void mqtt_reconnect() {
  while (!mqtt_client.connected()) {  // Loop until we're reconnected
    // Attempt to connect
    // If you do not want to use a username and password (anonymous connection), change next line to
